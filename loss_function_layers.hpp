...
struct	lf_l2{
	...
	float	backward(const	float	*inp,	const	float	*target,	unsigned	b=0){
		float	*gra=g(b),	*out=o(b);
		const	float	sca=1/sqrtf(input+1);	float	l=0;
		for(unsigned	i=0;	i<output;	i++){	out[i]-=target[i];	l+=out[i]*out[i];	out[i]*=sca*learning_rate;	}
		for(unsigned	i=0;	i<=input;	i++){
...
struct	lf_l1{
	...
	float	backward(const	float	*inp,	const	float	*target,	unsigned	b=0){
		float	*gra=g(b),	*out=o(b);
		const	float	sca=1/sqrtf(input+1);	float	l=0;
		for(unsigned	i=0;	i<output;	i++){	l+=fabsf(out[i]-target[i]);	out[i]=(out[i]>target[i]?1:-1)*sca*learning_rate;	}
		for(unsigned	i=0;	i<=input;	i++){
			float	s=(i<input?inp[i]:1),	*p=w(i),	z=0;
			for(unsigned	j=0;	j<output;	j++){	z+=p[j]*out[j];	p[j]-=s*out[j];	}
			if(i<input)	gra[i]=z;
...
struct	lf_softmax{
	...
	float	backward(const	float	*inp,	const	unsigned	target,	unsigned	b=0){
		float	*gra=g(b),	*out=o(b);
		const	float	sca=1/sqrtf(input+1);	float	l=-logf(fmaxf(out[target],FLT_MIN));
		for(unsigned	i=0;	i<output;	i++)	out[i]=(out[i]-(i==target))*sca*learning_rate;
		for(unsigned	i=0;	i<=input;	i++){
			float	s=(i<input?inp[i]:1),	*p=w(i),	z=0;
			for(unsigned	j=0;	j<output;	j++){	z+=p[j]*out[j];	p[j]-=s*out[j];	}
			if(i<input)	gra[i]=z;
